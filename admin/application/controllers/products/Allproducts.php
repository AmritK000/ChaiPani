...
class Allproducts extends CI_Controller {
	...
	{		
		...
		if($this->input->post('SaveChanges')):
			...
			$this->form_validation->set_rules('clossingSoon', 'Clossing Soon', 'trim');
			$this->form_validation->set_rules('validuptodate', 'Valid Upto Date', 'trim');
			$this->form_validation->set_rules('validuptotime', 'Valid Upto Time', 'trim');
...
$start = 2;
	foreach($data as $d){
		...
		$sheet->setCellValue('I'.$start, date('d-m-Y H:i A', strtotime($d['created_at'])));
		/*$sheet->setCellValue('I'.$start, $d['creation_ip']);
		$sheet->setCellValue('J'.$start, $d['vaccination_status']);*/

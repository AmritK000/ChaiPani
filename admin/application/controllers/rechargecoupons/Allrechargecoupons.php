...
class Allrechargecoupons extends CI_Controller {
	...
	{		
		$data['error'] 						= 	'';
		$data['activeMenu'] 				= 	'rechargecoupons';
			...
			if($this->form_validation->run() && $error == 'NO'): 
				...
				if($this->input->post('CurrentDataID') ==''):
					...
					for($i=0; $i < $qty; $i++){
						...
						$check 	= 	$this->common_model->checkDuplicate('da_coupon_code_only',$whereCon);
						if($check == 0){
							$data['rc_id']			= 	$param['rc_id'];
							$data['coupon_code'] 	= 	$whereCon['coupon_code'];
							$data['created_date']	=	$param['creation_date'];
							$this->common_model->addData('da_coupon_code_only',$data);
						}else{
							goto A;
						}
					}
	...
	************************************************************************/
	...
	{  
		...
		$sheet->setCellValue('B1', 'RECHARDE COUPON ID');

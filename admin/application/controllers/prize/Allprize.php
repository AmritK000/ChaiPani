...
class Allprize extends CI_Controller {
	...
	{	
       ...
       if($this->uri->segment(getUrlSegment())):
		...
		$this->layouts->admin_view('prize/allprize/index',array(),$data);
	}	// END OF FUNCTION
	...
	{		
		...
		if($editId):
			...
			$data['EDITDATA']				=	$this->common_model->getDataByParticularField('da_prize','prize_id',(int)$editId);//echo '<pre>';print_r($data['EDITDATA']);die;
		else:
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'): 
				...
				//print_r($this->input->post('product_id')); die();
				$param['product_id']		= 	addslashes($productData[0]);
		...
		$this->layouts->admin_view('prize/allprize/addeditdata',array(),$data);
	}	// END OF FUNCTION	
	...
	************************************************************************/
	...
	{  
		...
		$this->common_model->editData('da_prize',$param,'prize_id',(int)$changeStatusId);
	...
	************************************************************************/
	...
	{  
		...
		$wcon['where']          =   '';
		$data        			=   $this->common_model->getData('multiple','da_prize',$wcon);//echo '<pre>';print_r($data);die;
	...
	$start = 2;
		foreach($data as $d){
			...
			$sheet->setCellValue('B'.$start, $d['prize_seq_id']);

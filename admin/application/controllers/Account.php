...
class Account extends CI_Controller {
	...
	{	
		...
		$data['userData']       	=   $this->common_model->getData('multiple','da_users','',$sshortField,'3');

		$sField             		=   array('products_id'=>-1);
		$data['products']     		=   $this->common_model->getData('multiple','da_products','',$sField,'3');

		$shortField 				= 	array('membership_id'=>-1);
		$data['membership']   		=   $this->common_model->getData('multiple','da_membership','',$shortField,'3');

		$ashortField 				= 	array('appointment_id'=>-1);
		$data['loadBalance']    =   $this->common_model->getData('multiple','da_loadBalance','',$ashortField,'3');
		/* End */


		/* Chart Count */
		$data['usercount']    		=   $this->common_model->getData('count','da_users');

		$data['productCount']     	=   $this->common_model->getData('count','da_products');

		//$ppField['where']        	=   array('order_step'=>(int)4);
		$data['ordercount'] 		=   $this->common_model->getData('count','da_orders');

		$data['rechargeCount']   =   $this->common_model->getData('count','da_loadBalance');
		/* End */

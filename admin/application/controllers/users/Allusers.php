...
class Allusers extends CI_Controller {
	...
	************************************************************************/
	...
	{  
		...
		$wcon['where']          =   '';
		$data        			=   $this->common_model->getData('multiple','da_users',$wcon);
		...
		$sheet->setCellValue('F1', 'TOTAL ARABIAN POINTS');
		$sheet->setCellValue('G1', 'AVAILABLE ARABIAN POINTS');
	...
	$start = 2;
		foreach($data as $d){
			...
			$sheet->setCellValue('D'.$start, $d['users_mobile']);
			$sheet->setCellValue('E'.$start, $d['users_email']);
			$sheet->setCellValue('F'.$start, $d['totalArabianPoints']);
			$sheet->setCellValue('G'.$start, $d['availableArabianPoints']);
			$sheet->setCellValue('H'.$start, $d['users_type']);
			$sheet->setCellValue('I'.$start, date('d-M-Y ', $d['created_at']));
			if($d['status'] == 'A'){
				$sheet->setCellValue('J'.$start, ['Active']);	
			}else{
				$sheet->setCellValue('J'.$start, ['Inactive']);	
			}

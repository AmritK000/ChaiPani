		...
		if($this->input->post('loginFormSubmit')):	
			...
			if($this->form_validation->run()):	
				...
				if($result): 
					...
					//	$param['admin_password_otp']		=	(int)'1234';//(int)generateRandomString(4,'n');
					
						$this->common_model->editData('hcap_admin',$param,'admin_id',(int)$result['admin_id']);
						//$this->sms_model->sendLoginOtpSmsToUser($result['admin_phone'],$param['admin_password_otp']);
						//$this->session->set_userdata(array('otpType'=>'Login','otpAdminId'=>$result['admin_id'],'otpAdminMobile'=>$result['admin_phone']));
	...
	{	
		...
		if($this->input->post('otpVerificationFormSubmit')):	
			...
			if($this->form_validation->run()):	
				...
				$result		=	$this->admin_model->checkPin((int)$this->input->post('userOtp'));

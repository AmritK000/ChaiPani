...
class Disclaimer extends CI_Controller {
	...
	{	
       ...
       if ($this->uri->segment(getUrlSegment())):
		...
		$this->layouts->set_title('Disclaimer | CMS | Dealz Arabia');
	...
	{		
		...
		if($editId):
			...
			$data['EDITDATA']		=	$this->common_model->getDataByParticularField('da_cms','disclaimer_id',(int)$editId);
		else:
		...
		if($this->input->post('SaveChanges')): 
			...
			if($this->form_validation->run() && $error == 'NO'):  
				...
				if($this->input->post('CurrentDataID') ==''):
					$param['disclaimer_id']		=	(int)$this->common_model->getNextSequence('da_cms');
					...
					$this->common_model->editData('da_cms',$param,'disclaimer_id',(int)$NotificationTempId);
					$this->session->set_flashdata('alert_success',lang('updatesuccess'));
				endif;
				
				redirect(correctLink('CMSDISCLAIMER',$this->session->userdata('HCAP_ADMIN_CURRENT_PATH').$this->router->fetch_class().'/index'));
			endif;
		endif;
		
		$this->layouts->set_title('Add/Edit Disclaimer | CMS | Dealz Arabia');
	...
	{  
		...
		$this->common_model->editData('da_cms',$param,'disclaimer_id',(int)$changeStatusId);
	...
	{  
		...
		$this->common_model->deleteData('da_cms','disclaimer_id',(int)$deleteId);

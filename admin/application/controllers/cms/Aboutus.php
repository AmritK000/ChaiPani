...
class Aboutus extends CI_Controller {
	...
	{	
       ...
       if($this->uri->segment(getUrlSegment())):
		...
		$this->layouts->set_title('About us | CMS | Dealz Arabia');
	...
	{		
		...
		if($editId):
			...
			$data['EDITDATA']				=	$this->common_model->getDataByParticularField('da_cms','about_id',(int)$editId);
		else:
		...
		if($this->input->post('SaveChanges')):
			...
			if($this->form_validation->run() && $error == 'NO'): 
				...
				if($this->input->post('CurrentDataID') ==''):
					$param['about_id']			=	(int)$this->common_model->getNextSequence('da_cms');
					...
					$this->common_model->editData('da_cms',$param,'about_id',(int)$aboutId);

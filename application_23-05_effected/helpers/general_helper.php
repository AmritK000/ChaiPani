...
if (!function_exists('generateProductShareUrl')) {
	function generateProductShareUrl($products_id,$DZL_USERID,$DZL_USERS_REFERRAL_CODE) {
		$userDetails = base64_encode($DZL_USERID.'_'.$DZL_USERS_REFERRAL_CODE);
		return base_url('product-details/'.base64_encode($products_id)).'/'.$userDetails;
	}
}
...
if (!function_exists('pagesListData')) {
	function pagesListData() {
		$pagesArray			=	array('Homepage'			=>	"Homepage Banner",
